...
template <typename U, typename V>
static gb_inline U const &bit_cast(V const &v) { return reinterpret_cast<U const &>(v); }
...
static gb_inline void *align_formula_ptr(void *ptr, isize align) {
	uintptr result = (cast(uintptr)ptr) + align-1;
	return (void *)(result - result%align);
}


static BlockingMutex global_memory_block_mutex;

static void platform_virtual_memory_init(void);
...
static isize DEFAULT_PAGE_SIZE = 4096;

static MemoryBlock *virtual_memory_alloc(isize size);
static void virtual_memory_dealloc(MemoryBlock *block);
static void *arena_alloc(Arena *arena, isize min_size, isize alignment);
...
static std::atomic<isize> global_platform_memory_total_usage;
static PlatformMemoryBlock global_platform_memory_block_sentinel;

static PlatformMemoryBlock *platform_virtual_memory_alloc(isize total_size);
static void platform_virtual_memory_free(PlatformMemoryBlock *block);
static void platform_virtual_memory_protect(void *memory, isize size);
...
static GB_ALLOCATOR_PROC(arena_allocator_proc);
...
static Arena default_permanent_arena = {nullptr, DEFAULT_MINIMUM_BLOCK_SIZE};
static Arena default_temporary_arena = {nullptr, DEFAULT_MINIMUM_BLOCK_SIZE};


static Arena *get_arena(ThreadArenaKind kind) {
...
static GB_ALLOCATOR_PROC(thread_arena_allocator_proc) {
	void *ptr = nullptr;
...
static GB_ALLOCATOR_PROC(heap_allocator_proc);
...
static std::atomic<isize> total_heap_memory_allocated;
...
template <typename T>
static isize resize_array_raw(T **array, gbAllocator const &a, isize old_count, isize new_count, isize custom_alignment=1) {
