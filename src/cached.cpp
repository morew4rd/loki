...
static bool recursively_delete_directory(wchar_t *wpath_c) {
...
static bool recursively_delete_directory(String const &path) {
#if defined(GB_SYSTEM_WINDOWS)
...
static bool try_clear_cache(void) {
	return recursively_delete_directory(str_lit(".odin-cache"));
}


static u64 crc64_with_seed(void const *data, isize len, u64 seed) {
	isize remaining;
	u64 result = ~seed;
...
static bool check_if_exists_file_otherwise_create(String const &str) {
...
static bool check_if_exists_directory_otherwise_create(String const &str) {
#if defined(GB_SYSTEM_WINDOWS)
...
static bool try_copy_executable_cache_internal(bool to_cache) {
...
static bool try_copy_executable_to_cache(void) {
...
static bool try_copy_executable_from_cache(void) {
...
static bool try_cached_build(Checker *c, Array<String> const &args) {
	TEMPORARY_ALLOCATOR_GUARD();
